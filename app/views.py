...
class Skill(Resource):
    ...
    def get(cls, job_role):
        ...
        r = requests.get(url = "http://34.121.45.137:8080/data?job_role={job_role}&skill_type=both skill".format(job_role=job_role))
...
class SoftSkill(Resource):
    ...
    def get(cls, job_role):
        ...
        job_role = job_role.strip()
        job_role = job_role.replace(" ", "_")
        r = requests.get(url = "http://34.121.45.137:8080/data?job_role={job_role}&skill_type=soft skill".format(job_role=job_role))
...
class HardSkill(Resource):
    ...
    def get(cls, job_role):
        ...
        job_role = job_role.strip()
        job_role = job_role.replace(" ", "_")
        r = requests.get(url = "http://34.121.45.137:8080/data?job_role={job_role}&skill_type=hard skill".format(job_role=job_role))
